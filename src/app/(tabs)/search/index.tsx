      ...
      const searchApps = async () => {
        ...
        try {
          const response = await fetch(
            `https://itunes.apple.com/search?term=${encodeURIComponent(query)}&entity=software&limit=20`
          );
  ...
  const renderAppItem = ({ item }: { item: any }) => (
    <TouchableOpacity style={styles.appItem} onPress={() => {}}>
      ...
      <View style={styles.appInfo}>
        ...
        <ThemedText type="small" numberOfLines={1} style={styles.appSub}>
          {item.primaryGenreName} • {item.sellerName}
        </ThemedText>
      </View>
          ...
          !query ? (
            <View style={styles.suggestionsContainer}>
              <View style={styles.header}>
                <ThemedText type="title" style={[styles.pageTitle, { color: colors.text }]}>
                  Search
                </ThemedText>
              </View>
                    ...
                    <GlassCard style={styles.suggestedCard}>
                      <View style={styles.suggestedContent}>
                        ...
                        <View style={styles.suggestedInfo}>
                          ...
                          {app.ad ? (
                            <View style={styles.adTag}>
                              <ThemedText style={styles.adTagText}>Ad</ThemedText>
                            </View>
                          ) : null}
                        </View>
                        <View style={styles.suggestedAction}>
                          ...
                          <ThemedText type="small" style={styles.inAppText}>
                            In-App Purchases
                          </ThemedText>
                        </View>
                      </View>
                ...
                {BROWSE_CATEGORIES.map((cat) => (
                  <TouchableOpacity key={cat.id} activeOpacity={0.8} style={[styles.browseCard, { backgroundColor: cat.color }]}>
                    ...
                    <View style={styles.browseIconContainer}>
                      <SymbolView name={cat.icon as any} size={42} tintColor="rgba(255,255,255,0.25)" />
                    </View>
                  </TouchableOpacity>
                ))}
              </View>
...
const styles = StyleSheet.create({
  ...
  pageTitle: { fontSize: 34, fontWeight: '800', letterSpacing: -1.2 },
  listContent: { paddingHorizontal: Spacing.four, paddingBottom: 180 },
  suggestionsContainer: { marginBottom: 20 },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 12, marginTop: 4 },
  sectionTitle: { fontSize: 22, fontWeight: '700', letterSpacing: -0.6 },
  horizontalScroll: { paddingRight: Spacing.four, gap: 12, paddingVertical: 4 },
  suggestedCardContainer: { width: width * 0.88, marginRight: 12 },
  suggestedCard: {
  ...
  suggestedContent: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  suggestedIcon: {
    width: 62,
    height: 62,
    borderRadius: 14,
  ...
  suggestedAction: { alignItems: 'center', gap: 4 },
  inAppText: { fontSize: 8, opacity: 0.4, fontWeight: '500' },
  adTag: { backgroundColor: 'rgba(0,122,255,0.1)', alignSelf: 'flex-start', paddingHorizontal: 6, paddingVertical: 1, borderRadius: 4, marginTop: 4 },
  adTagText: { fontSize: 10, fontWeight: '700', color: '#007AFF' },
  suggestedGetBtn: { backgroundColor: '#F0F0F7', paddingHorizontal: 18, paddingVertical: 6, borderRadius: 18, minWidth: 68, alignItems: 'center' },
  browseGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, justifyContent: 'space-between' },
  browseCard: {
  ...
  browseCardTitle: { color: '#FFF', fontSize: 16, fontWeight: '700', lineHeight: 20 },
  browseIconContainer: { position: 'absolute', bottom: -6, right: -6, transform: [{ rotate: '-12deg' }] },
  appItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: 'rgba(0,0,0,0.05)', gap: 12 },
  appIcon: { width: 64, height: 64, borderRadius: 14, backgroundColor: '#F2F2F7' },
  appInfo: { flex: 1 },
  appSub: { opacity: 0.5, fontSize: 13 },
  appBtn: { backgroundColor: '#F2F2F7', paddingHorizontal: 18, paddingVertical: 7, borderRadius: 20 },
});
